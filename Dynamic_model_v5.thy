...
locale SM =
  ...
  assumes 
    ...
    policy_respect: "\<forall>v u s t. reachable0 s \<and> reachable0 t \<and> (s \<sim> u \<sim> t)
                 ...
                 have a3: "sources (b # bs) u s = sources (b # bs) u t"
                   ...
                   next
                     ...
                     have b4: "\<forall>v. v\<in>sources bs u (step s b)\<longrightarrow>\<not> interferes (the (domain b)) t v "
                       using b1 b3 p9 policy_respect by fastforce
          ...
          next
            ...
            then show ?case
              proof -
              {
                 ...
                 have a3: "sources (b # bs) u s = sources (b # bs) u t"
                   using p1 p2 p3 p4 p5 p9 sources_eq1 by blast
...
subsection{* Interference framework of information flow security properties *}
    ...
    lemma noninf_imp_pr: "noninfluence \<Longrightarrow> policy_respect"
      proof -
        assume p0: "noninfluence"
        have p1: "\<forall> d as s t. reachable0 s \<and> reachable0 t
                  \<and> (s \<approx> (sources as d s) \<approx> t)
        ...
        have p2: "\<forall>a d s t. reachable0 s \<and> reachable0 t \<and> (s \<sim> d \<sim> t) \<and> 
                           (((the (domain a)) @ s \<leadsto> d) \<longrightarrow> (s \<sim> (the (domain a)) \<sim> t))
                           \<longrightarrow> ((step s a) \<sim> d \<sim> (step t a))"
          using p0 noninf_imp_sc by auto
        have p3: "\<forall>a d s. reachable0 s \<and> \<not>((the (domain a)) @ s \<leadsto> d) 
                          \<longrightarrow> (s \<sim> d \<sim> (step s a))"
          using p0 noninf_imp_dlr by auto
        have "\<forall>v u s t. reachable0 s \<and> reachable0 t \<and> (s \<sim> u \<sim> t)
                  \<longrightarrow> (interferes v s u = interferes v t u)"
          proof -
          {
            fix a d s
    ...
    theorem UnwindingTheorem1 : "\<lbrakk>weakly_step_consistent;
                            dynamic_local_respect;
                            policy_respect\<rbrakk>  \<Longrightarrow> noninfluence"
      using UnwindingTheorem weak_with_step_cons by blast

   (*
     noninfluence \<Rightarrow> policy_respect cannot be proved
   *)
   theorem uc_eq_noninf : "(step_consistent \<and> dynamic_local_respect \<and> policy_respect) = noninfluence"
     oops
   ...
   theorem noninf_impl_weak:"noninfluence \<Longrightarrow> weak_noninfluence"
     ...
     have "\<forall> d as bs s t . reachable0 s \<and> reachable0 t \<and> (s \<approx> (sources as d s) \<approx> t) 
       ...
       proof -
       {
         ...
         assume a0: "reachable0 s"
         assume a1: "reachable0 t"
         assume a2: "(s \<approx> (sources as d s) \<approx> t)"
         assume a3: "ipurge as d s = ipurge bs d s"
         assume a5: "ipurge as d t = ipurge bs d t"
         have a4: "noninterference_r"
           using noninf_impl_nonintf_r p0 by auto
         have b1: "(s \<lhd> as \<cong> t \<lhd> (ipurge as d t) @ d)"
           using a0 a1 a2 p1 by auto
         have b2: "(s \<lhd> as \<cong> s \<lhd> (ipurge as d s) @ d)"
           using a0 a4 by auto
         have b3: "(t \<lhd> as \<cong> t \<lhd> (ipurge as d t) @ d)"
           using a1 a4 by auto
         have b4: "(s \<lhd> as \<cong> t \<lhd> as @ d)"
           using a0 a1 a2 noninf_imp_sc nonleakage_def p0 sc_imp_nonlk by blast
         have b5: "(t \<lhd> bs \<cong> t \<lhd> (ipurge bs d t) @ d)"
           using a1 a4 by auto
         have b6: "(t \<lhd> bs \<cong> t \<lhd> (ipurge as d t) @ d)"
           using b5 a5 by auto
         have b7: "(s \<lhd> as \<cong> t \<lhd> bs @ d)"
           using a1 a4 a5 b4 nonintf_r_impl_wk_nonintf_r observ_equiv_trans weak_noninterference_r_def by blast
      ...
      proof -
        ...
        have "\<forall> d as bs s t . reachable0 s \<and> reachable0 t \<and> (s \<approx> (sources as d s) \<approx> t) 
          ...
          proof -
          {
            ...
            have a2: "ipurge as d s = ipurge as d t"
