    ...
    "vpeq1 s d t \<equiv> 
        ...
        in
          if(d1 = d2 \<and> ps1 = ps2
             \<and> (\<forall>v. \<not>((get_t_set s v) \<subseteq> get_t_set s d) \<longleftrightarrow> \<not>((get_t_set t v) \<subseteq> get_t_set t d)))
          then
            True
          else
            False
...
lemma vpeq1_get_domain_eq: "vpeq1 s d t \<Longrightarrow> get_domain_from_st_by_id s d = get_domain_from_st_by_id t d"
  by (meson vpeq1_def)

lemma vpeq1_get_domain_cfg_ports_eq: "vpeq1 s d t \<Longrightarrow> get_domain_cfg_ports s d = get_domain_cfg_ports t d"
  by (meson vpeq1_def)

lemma vpeq1_non_interf_eq: "vpeq1 s d t \<Longrightarrow> (\<forall>v. \<not>((get_t_set s v) \<subseteq> get_t_set s d) \<longleftrightarrow> \<not>((get_t_set t v) \<subseteq> get_t_set t d))"
  by (meson vpeq1_def)
